...
1. In **Settings → Pages**: Deploy from branch `gh-pages`, root.
2. Set **Custom domain** to `charts.preloop.ai`.
3. Add a CNAME record: `charts.preloop.ai` → `preloop.github.io`.

## Automation

The index is updated automatically when [preloop/preloop](https://github.com/preloop/preloop) creates a release. The preloop release workflow triggers this repo via `repository_dispatch`.
